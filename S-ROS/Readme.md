...
1) Prove that a spiking neural network can learn procedural, syntactic code execution without pre-training or distillation.

2) Demonstrate real-time, associative memory retrieval for robotic control.

3) Establish a biologically grounded neuro-symbolic bridge that avoids the probabilistic failure modes of dense LLMs.
