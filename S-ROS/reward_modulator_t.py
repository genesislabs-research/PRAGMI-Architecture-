...
def _run_self_tests() -> None:
    ...
    try:
        ...
        mod = RewardModulator(cfg)
        # With baseline_decay=0.95, convergence timescale is ~20 steps;
        # 100 iterations guarantees baseline reaches ~0.994 so d[-1] < 0.01.
        d_values = [mod.compute_rpe(predicted=0.0, actual=1.0) for _ in range(100)]
        ...
        print(
            ...
            f"{d_values[0]:.4f} to {d_values[-1]:.4f} over 100 identical rewards."
        )
    except AssertionError as e:
        print(
