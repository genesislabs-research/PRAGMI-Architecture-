...
def run_tests() -> bool:
    ...
    # ------------------------------------------------------------------
    # Test 19: DA signal source verification
    # Verifies that the DA signal is derived from CA1 novelty, not from
    # the normalized loss proxy. Runs 10 forward passes, computes the
    # correct DA value from novelty, and confirms it matches what would
    # be passed to neuromod.update_da().
    # ------------------------------------------------------------------
    try:
        cfg_t19 = CognitiveKernelConfig()
        kernel19 = CognitiveKernel(cfg_t19)
        enc19 = LightweightEncoder(vocab_size=50257, d_model=256, seq_len=128)
        proj19 = FixedCoordinateProjection(input_dim=256, output_dim=64, seed=42)

        da_from_novelty_vals = []
        for _ in range(10):
            ids19 = torch.randint(0, 50257, (4, 128))
            emb19 = enc19(ids19)
            coords19 = proj19(emb19)
            recon19, novelty19, _ = kernel19(coords19)
            da_val = float(novelty19.mean().detach().item())
            da_val = max(0.0, min(1.0, da_val))
            da_from_novelty_vals.append(da_val)

        # All DA values must be in [0, 1] and not all identical (novelty varies).
        assert all(0.0 <= v <= 1.0 for v in da_from_novelty_vals), (
            "DA values derived from novelty are outside [0, 1]"
        )
        print(
            f"[PASS] 19/21  DA signal source: 10 steps, "
            f"da_mean={sum(da_from_novelty_vals)/len(da_from_novelty_vals):.4f}, "
            f"all in [0,1]"
        )
        passed += 1
    except Exception as e:
        print(f"[FAIL] 19/21  DA signal source: {e}")

    # ------------------------------------------------------------------
    # Test 20: Reconstruction feedback shape and gradient isolation
    # After one kernel forward pass, the reconstruction must have shape
    # (B, 64) and requires_grad must be False (detached for feedback).
    # ------------------------------------------------------------------
    try:
        cfg_t20 = CognitiveKernelConfig()
        kernel20 = CognitiveKernel(cfg_t20)
        proj20 = FixedCoordinateProjection(input_dim=256, output_dim=64, seed=42)
        enc20 = LightweightEncoder(vocab_size=50257, d_model=256, seq_len=128)

        ids20 = torch.randint(0, 50257, (4, 128))
        emb20 = enc20(ids20)
        coords20 = proj20(emb20)
        recon20, _, _ = kernel20(coords20)

        current_recon = recon20.detach()
        assert current_recon.shape == (4, 64), (
            f"Expected reconstruction shape (4, 64), got {current_recon.shape}"
        )
        assert not current_recon.requires_grad, (
            "Detached reconstruction must have requires_grad=False"
        )
        print(
            f"[PASS] 20/21  Reconstruction feedback: "
            f"shape={tuple(current_recon.shape)}, requires_grad=False confirmed"
        )
        passed += 1
    except Exception as e:
        print(f"[FAIL] 20/21  Reconstruction feedback: {e}")

    # ------------------------------------------------------------------
    # Test 21: CorticalBuffer state persistence across two forward passes
    # After the first pass updates the buffer, state must be nonzero.
    # After the second pass, state must remain nonzero (not reset between
    # steps). The buffer only resets on explicit reset() call, not on
    # sleep consolidation.
    # ------------------------------------------------------------------
    try:
        cb21_cfg = CorticalBufferConfig(d_model=256, buffer_dim=32)
        cb21 = CorticalBuffer(cb21_cfg)
        enc21 = LightweightEncoder(vocab_size=50257, d_model=256, seq_len=128)

        # Initial state must be zero.
        assert cb21.state.abs().sum().item() == 0.0, (
            "CorticalBuffer state must start at zero"
        )

        # After first update, state must be nonzero.
        ids21a = torch.randint(0, 50257, (4, 128))
        emb21a = enc21(ids21a)
        cb21.update(emb21a.detach())
        norm_after_first = cb21.state.norm().item()
        assert norm_after_first > 0.0, (
            f"Buffer state is still zero after first update: norm={norm_after_first}"
        )

        # After second update, state must remain nonzero.
        ids21b = torch.randint(0, 50257, (4, 128))
        emb21b = enc21(ids21b)
        cb21.update(emb21b.detach())
        norm_after_second = cb21.state.norm().item()
        assert norm_after_second > 0.0, (
            f"Buffer state zeroed after second update: norm={norm_after_second}"
        )

        print(
            f"[PASS] 21/21  CorticalBuffer persistence: "
            f"norm_after_first={norm_after_first:.4f}, "
            f"norm_after_second={norm_after_second:.4f}"
        )
        passed += 1
    except Exception as e:
        print(f"[FAIL] 21/21  CorticalBuffer persistence: {e}")
...
def train_small_core(
    ...
    # Compresses encoder output after each forward pass and injects it as
    # additive bias at the start of the next forward pass. This models the
    # persistent activity of prefrontal delay-period neurons that maintain
    # working memory across inter-trial intervals.
    ...
    # ReconstructionInjectionProjection: fixed random projection from 64-dim
    # kernel reconstruction space back to 256-dim encoder embedding space.
    # Allows the kernel's reconstructed coordinate to modulate the encoder's
    # representation on the next forward pass without creating a learned
    # circular dependency.
    #
    # This is Option A from the April 8 integration spec: additive injection
    # scaled by 0.1 (sub-threshold influence, not overriding the encoder's
    # own computation). The scale of 0.1 is an engineering approximation of
    # top-down modulatory influence from hippocampus to cortex.
    # NOT a biological quantity: the scale and seed are engineering choices.
    # Anatomical analog: CA1 -> subiculum -> entorhinal cortex -> neocortex
    # backprojection pathway. Suzuki WA, Amaral DG (1994).
    # DOI: 10.1002/cne.903500302
    recon_injection_proj = FixedCoordinateProjection(
        input_dim=64, output_dim=256, seed=99
    ).to(device)
    ...
    # Reconstruction feedback buffer. The kernel's reconstructed coordinate
    # vector from step N is injected additively into the encoder output at
    # step N+1. Initialized to None so the first step receives no injection.
    # Cleared on sleep consolidation (April 6 decision: sleep clears the
    # reconstruction buffer because the kernel has no short-term context
    # after consolidation and stale reconstructions would corrupt encoding).
    # NOT a biological quantity: the buffer mechanism is an engineering
    # implementation of the hippocampal-to-cortex backprojection pathway.
    # Anatomical analog: Suzuki WA, Amaral DG (1994). DOI: 10.1002/cne.903500302
    prev_reconstruction: Optional[torch.Tensor] = None
    ...
    for step in range(n_steps):
        ...
        # Change 3A (Priority 3): ACh is high during active encoding (wake).
        # Biological basis: acetylcholine concentration is elevated during
        # waking states, promoting bottom-up sensory encoding over top-down
        # retrieval. Hasselmo ME (1999). DOI: 10.1016/S0166-2236(99)01455-5
        # Value 0.8 is an engineering approximation, NOT a biological quantity.
        neuromod.update_ach(0.8)

        token_ids = _next_batch()  # (B, seq_len)
        embeddings = encoder(token_ids)  # (B, 256)

        # Change 1C: CorticalBuffer read and inject.
        # The buffer compresses the previous step's encoder output and injects
        # it as an additive bias before the coordinate projection. This models
        # PFC delay-period neurons maintaining context across inter-pass
        # intervals. The buffer starts at zero so the first step is unaffected.
        # Goldman-Rakic PS (1995). DOI: 10.1016/0896-6273(95)90304-6
        cb_injection = cortical_buffer.get_injection()  # (256,)
        if cb_injection.abs().sum().item() > 0.0:
            embeddings = embeddings + cb_injection.unsqueeze(0)

        # Change 1B: Reconstruction feedback injection.
        # The kernel's 64-dim reconstruction from the previous step is
        # projected back to 256-dim via a fixed random projection (seed=99)
        # and added to the encoder output at scale 0.1. This is the
        # hippocampal-to-cortex backprojection: the reconstructed episodic
        # context modulates the encoder's current representation.
        # Scale 0.1 is sub-threshold influence: it biases without overriding.
        # NOT a biological quantity: scale and projection seed are engineering.
        # Anatomical analog: CA1 -> subiculum -> EC -> neocortex backprojection.
        # Suzuki WA, Amaral DG (1994). DOI: 10.1002/cne.903500302
        if prev_reconstruction is not None:
            recon_bias = recon_injection_proj(prev_reconstruction)  # (B, 256)
            embeddings = embeddings + 0.1 * recon_bias
        ...
        # Change 1B: Store reconstruction for next step's injection.
        # Detached to break circular gradient dependency: Timmy's output
        # determines the kernel's input which determines Timmy's input.
        # Without detach, the optimizer would attempt to co-optimize through
        # the reconstruction path, creating an unstable feedback loop.
        # April 6, 2026 decision: reconstruction feedback is detached.
        current_reconstruction = reconstruction.detach()  # (B, 64)

        # Change 1C: CorticalBuffer update.
        # Compress the encoder embedding for use on the next step.
        # The buffer is updated AFTER the kernel forward (not after the encoder)
        # so it captures the full-context embedding including reconstruction
        # injection, giving the buffer richer state to maintain.
        # NOT reset on sleep (April 8 spec): the buffer is reconsolidated
        # during sleep, not cleared. This differs from prev_reconstruction.
        cortical_buffer.update(embeddings.detach())
        ...
        # ----------------------------------------------------------------
        # Step 7: Loss statistics for monitoring and 5-HT signal
        # ----------------------------------------------------------------
        raw_loss = kernel_loss.item()
        loss_history.append(raw_loss)

        # Update running EMA and variance for loss monitoring.
        # These statistics are retained as diagnostics. They no longer drive
        # the DA signal (replaced by CA1 novelty per April 6 decision).
        ...
        # Renamed from da_signal to loss_monitor_01. Retained for logging only.
        # NOT a biological quantity: engineering normalization diagnostic.
        loss_monitor_01 = float(torch.clamp(torch.tensor(-normalized_loss_signal * 0.5 + 0.5), 0.0, 1.0))

        # ----------------------------------------------------------------
        # Step 8: NeuromodulatorBroadcast updates
        # ----------------------------------------------------------------

        # Change 1A: Dopamine driven by CA1 novelty (April 6 decision, settled).
        # The CA1 comparator's novelty scalar is the biologically correct
        # surprise signal for dopamine. High novelty (novel experience) ->
        # high DA. Low novelty (familiar experience) -> low DA.
        # Replaces the normalized loss proxy (loss_monitor_01) which was
        # an engineering placeholder with no direct biological grounding.
        # Lisman JE, Grace AA (2005). DOI: 10.1016/j.neuron.2005.05.002
        da_signal = float(novelty.mean().detach().item())
        da_signal = max(0.0, min(1.0, da_signal))
        neuromod.update_da(da_signal)

        # Norepinephrine encodes unexpected uncertainty (world model variance).
        # Unchanged: ensemble variance measures model uncertainty, which is
        # distinct from episodic novelty (CA1). Both signals are needed.
        ...
        # Change 3B (Priority 3): Serotonin encodes loss stability (exploration/exploitation).
        # High stability (loss decreasing smoothly) -> high 5-HT -> exploit.
        # High volatility (loss increasing or erratic) -> low 5-HT -> explore.
        # Values derived from loss_var_ema which is already computed above.
        # NOT a biological quantity: stability proxy is an engineering approximation.
        # Dayan P, Huys QJ (2009). DOI: 10.1371/journal.pcbi.1000587
        loss_stability = 1.0 / (1.0 + math.sqrt(max(loss_var_ema, 1e-8)))
        ht_signal = max(0.0, min(1.0, loss_stability))
        neuromod.update_ht(ht_signal)

        # Update prev_reconstruction for next step.
        prev_reconstruction = current_reconstruction
        ...
        if (step + 1) % sleep_interval == 0:
            # Change 3A: ACh drops during sleep consolidation (encoding off).
            # Biological basis: ACh concentration falls during slow-wave sleep,
            # shifting the network from encoding to consolidation mode.
            # Hasselmo ME (1999). DOI: 10.1016/S0166-2236(99)01455-5
            # Value 0.2 is an engineering approximation, NOT a biological quantity.
            neuromod.update_ach(0.2)

            sleep_diag = kernel.sleep_consolidation()

            # Change 1B: Clear reconstruction buffer on sleep.
            # April 6 decision: the kernel has no short-term context after
            # consolidation. Stale reconstructions from before sleep would
            # corrupt the first encoding step of the new wake cycle.
            ...
            # Change 3A: Restore ACh to wake level after sleep completes.
            neuromod.update_ach(0.8)
        ...
        if (step + 1) % log_interval == 0:
            ...
            buffer_norm = cortical_buffer.state.norm().item() if cortical_buffer.state is not None else 0.0
            print(
                ...
                f"ne={ne_signal:.4f} | "
                f"eta={eta:.3f} | "
                f"novelty_mean={novelty.mean().item():.4f} | "
                f"maturity={neuromod.global_maturity.item():.4f} | "
                f"buffer_norm={buffer_norm:.4f} | "
                f"episodes={diagnostics.get('num_stored_episodes', 0)} | "
                f"loss_monitor={loss_monitor_01:.4f} | "
